...
export default function Home() {
  ...
  const loadDashboard = async () => {
    ...
    setRecurring((recurringData as Recurring[]) ?? [])
    setDebts((debtsData as any[]) ?? [])
  }
  ...
  const metrics = useMemo(() => {
    ...
    const deuda = accounts
      ...
      .reduce((acc, a) => acc + Number(a.current_balance || 0), 0) +
      debts.reduce((acc, d) => acc + Number(d.current_balance || 0), 0)
    ...
    const fixedExpense = recurring.reduce((acc, r) => acc + Number(r.amount || 0), 0)

    return { disponible, deuda, totalIncome, totalExpense, fixedExpense, flow: totalIncome - totalExpense }
  }, [accounts, monthTransactions, recurring, debts])
  ...
  const categoryChartData = useMemo(() => {
    ...
    monthTransactions
      .filter(tx => ['expense', 'credit_card_purchase'].includes(tx.transaction_type))
      .forEach(tx => {
        const name = tx.category_id ? (catMap.get(tx.category_id) || 'Otros') : 'Sin categoría'
        counts.set(name, (counts.get(name) || 0) + Number(tx.amount))
      })
    ...
    <main className="min-h-screen bg-slate-50 pb-12">
      ...
      <section className="max-w-7xl mx-auto px-6 -mt-8 relative z-20">
        ...
        <div className="grid gap-6 md:grid-cols-4 mb-8">
          ...
          <KpiCard title="In vs Out (Mes)" value={formatMoney(metrics.flow)} valueClassName={metrics.flow >= 0 ? 'text-emerald-600' : 'text-rose-600'} />
          <KpiCard title="Deuda Total" value={formatMoney(metrics.deuda)} valueClassName="text-rose-600" />
          <KpiCard title="Fijos Estimados" value={formatMoney(metrics.fixedExpense)} valueClassName="text-slate-500" />
        </div>

        {/* Gráficas Inteligentes */}
        <div className="grid gap-6 lg:grid-cols-12 mb-8">
          <div className="lg:col-span-8">
            <Panel title="Flujo de Caja" subtitle="Ingresos vs Gastos del mes actual">
                  ...
                  <div key={tx.id} className="group flex items-center justify-between p-4 rounded-3xl border border-slate-50 bg-white hover:border-slate-200 hover:shadow-lg transition-all">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">{formatDate(tx.transaction_date)}</p>
                      </div>
                    </div>
